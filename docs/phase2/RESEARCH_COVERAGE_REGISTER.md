...
This register makes the research basis for Phase 2 test and claim design auditable. The documented full screen of the [OpenAI Research Index](https://openai.com/research/index/) and the named Anthropic research program remains current through **2026-08-14**. A targeted Phase 2.5 source check was performed on 2026-08-15, but it did not repeat the complete procedure below and therefore does not advance the full-screen date. A complete claim-class refresh remains required before alpha.6 evidence publication. Screening covers material that could change this project's threat model, evidence standard, evaluation validity, control architecture, monitoring plan, human-authorization boundary, or public wording.
...
| [Internal coding-agent misalignment monitoring](https://openai.com/index/how-we-monitor-internal-coding-agents-misalignment/) | Evaluate long, tool-rich trajectories; quantify recall/false negatives, latency, escalation coverage, human response, privacy, and actor-monitor collusion; do not convert perfect easy-control results into an open-ended false-negative bound | ADV-009 and ADV-011 trace these measures; `P2-CE-005` is deliberately limited to fixed deterministic controls and permits no monitoring-efficacy inference | Planned |
