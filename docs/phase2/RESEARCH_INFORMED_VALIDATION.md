...
6. **Separate implementation evidence from behavioral evidence.** The 143-test suite can support CE-1 implementation conformance for the identified checkout. It is not an observed `P2-CE-004` denominator, repeatability result, or CE-2 claim.
...
| Independent outcome checking should not reuse the actor's calculation | Separately implemented in-process projection of 20 feature values and traces, with prohibited production imports | CE-1 implementation conformance in the 143-test local suite; not external independence |
