...
6. **Separate implementation evidence from behavioral evidence.** The version-bound alpha.5 suite supports its recorded CE-1 implementation statement. The predecessor alpha.6 design-freeze implementation passed 193/193 tests in a review-local run, and CI succeeded for exact Commit `08ce203c` on 2026-08-15, supporting CE-1 implementation conformance for that commit only. This package candidate's Phase 2.5 technical suite passed 222/222; the separate public-site module passed 9/9, producing a 231/231 repository aggregate without extending the research-informed or Phase 2.5 evidence claim. GitHub CI remains required on the exact published package commit. Neither state is a tagged release, an evidence package, or `P2-CE-005` behavior evidence. The separate `P2-CE-004` artifacts, denominator, repeatability result, and claim record support only their exact CE-2 wording. A `P2-CE-005` plan or green suite does not create CE-2 behavior evidence.
