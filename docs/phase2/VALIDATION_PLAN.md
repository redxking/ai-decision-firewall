...
The current evidence base contains only synthetic fixtures with `historical_case_count=0`: the three-case starter, the seven-record Phase 2.1 qualification campaign, and the ephemeral test-only inputs used by the fixed Phase 2.3 Gate B campaign. Phase 2.4 adds CE-1 implementation tests for typed/source-authorized features and reference projection. Its corrected `P2-CE-004` freeze supplies no valid published result evidence; a prior unpublished package was invalidated after its validator failed a non-finite-JSON negative control and is excluded. Historical efficacy, historical calibration, historical acceptance rates, and analyst-agreement claims are therefore unavailable.
