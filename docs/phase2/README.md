...
| `src/adf_poc/replay/reference_decision.py` | Separately recompute the deterministic evidence, model, policy, verifier, and read-only final-decision surfaces from frozen bytes without importing production calculation paths |
| `contracts/v0.2.0/source-to-decision-assurance.schema.json` | Closed metadata-only matched source-to-decision receipt |
| `config/source_to_decision_ce2_campaign_plan.json` | Fixed 20-attempt-per-run `P2-CE-005` plan and expected outcomes; CE-0, not observed evidence |
| `contracts/v0.2.0/source-to-decision-ce2-campaign.schema.json` | Closed plan/profile/result/summary shapes for the planned `P2-CE-005` campaign |
| `tests/test_reference_decision.py` | Positive and stage-specific reference-path, parser, numeric, case-binding, and import-separation tests |
...
| `src/adf_poc/replay/gate_b_oracle.py` | Closed causal-identity registry and exact tuple matching for 25 selected Gate B failures; unclassified failures remain unscorable |
| `src/adf_poc/replay/payload_observer.py` | Bounded test observer for governed payload-role access through enumerated Python file APIs; not a sandbox or reference monitor |
| `tests/test_gate_b_failure_oracle.py` and `tests/test_payload_observer.py` | Selected causal-identity/oracle and pre-payload observation tests; 24 selected pre-payload mutations plus one post-qualification identity; no campaign claim |
| `run_poc.py` and `tests/test_run_poc_safety.py` | Ordinary/local and explicit-freeze write interlock, generated-leaf checks, and seven-output local-manifest binding; 14 focused tests; not OS-level confinement |
...
14. Run the separately implemented in-process feature projector against normalized cases and serialized decisions and require one exact 20-feature value/trace match per case.
15. From the frozen normalized-case, serialized-decision, model, and policy bytes, separately recompute and compare `EVIDENCE`, `MODEL`, `POLICY`, `VERIFIER`, and `FINAL_SURFACE` in that order. Production and reference evidence aggregates use ordered `math.fsum(values) / event_count`; model calculations use ordered `math.fsum` before the intercept and clamped sigmoid. A mismatch emits neither reference artifact and stops before qualification/rejection publication, adjudication decoding, comparisons, metrics, or completed-run finalization; earlier normalized, decision, and audit files remain incomplete diagnostic material only.
16. Validate complete, unique, deterministically ordered, cross-bound receipt sets; then write and freeze both metadata-only reference-assurance artifacts.
17. Only after both reference checks succeed, publish qualification/rejection artifacts when enabled and materialize and decode the frozen adjudications for comparison; ambiguous duplicate JSON members fail rather than using last-member-wins semantics.
18. Reverify the complete snapshot, current authorization, and every bound replay artifact; produce comparisons and metrics; construct the run manifest; revalidate artifacts; write the manifest; and perform final checks before returning success. Artifact or manifest presence after an exception does not establish completion.
...
The Phase 2.4 typed-feature and reference-projection controls are CE-1 implementation-conformance evidence in the recorded checkout. Phase 2.5 extends the separate calculation through evidence assessment, model probability/factors, policy/disposition, verifier checks and downgrade behavior, and the bounded read-only final surface. Both paths are same-process, same-project checks, not an external oracle or independent replication. Agreement does not establish source truth, outcome correctness, policy fitness, operational efficacy, external custody, or organizational independence.

The committed `P2-CE-001` and `P2-CE-002` bundles were created before alpha.5 and contain neither current reference-assurance artifact. Their original narrow claims remain validated only under their recorded version and artifact sets; later controls do not retroactively upgrade them. Every newly generated alpha.6 replay must emit and bind both `reference_feature_assurance.jsonl` and `source_to_decision_assurance.jsonl`, and the harness must return successfully after final revalidation, to be complete.

`P2-CE-005-SOURCE-TO-DECISION-SYNTHETIC` remains CE-0 `NOT_EVALUATED`. The plan specifies ten clean/mutant pairs per run and two deterministic same-process repetitions: 40 planned attempt observations, zero retries/exclusions, and two expected blocks at each of `EVIDENCE`, `MODEL`, `POLICY`, `VERIFIER`, and `FINAL_SURFACE` per run. A single directly instrumented production baseline is shared by each clean/mutant twin: ten production baselines and ten calls each to the engine, evidence, model, policy, and verifier per run, 20 baselines and 20 calls to each component across the campaign. The reference path is invoked for every attempt: 20 calls per run, 40 total. Every mutant is designed to pass the legacy read-only decision, eight-stage audit, and Phase 2.4 feature checks before the Phase 2.5 detector runs. The result contract separates direct authorization-gate/broker/target-effect/scoped-write counters from authorization-token, action-result, and operational-effect values derived from the serialized decision. All counts and expected zeros are planned controls, not observations or evidence.
...
Validate only the planned `P2-CE-005` structure and bindings after the campaign generator is frozen:

```bash
python3 scripts/generate_source_to_decision_ce2_campaign.py --validate-plan
```

This command does not execute the campaign and cannot create CE-2 evidence. The campaign remains CE-0 `NOT_EVALUATED` until an exact clean Commit-A execution and separate evidence-only Commit B satisfy the documented freeze protocol.

Use `--validate-only` to verify configuration, manifest integrity, attestations, record qualification, and case contracts without invoking the engine. A successful alpha.6 qualification run adds deterministic `qualification_accounting.jsonl`, `rejections.jsonl`, `reference_feature_assurance.jsonl`, and `source_to_decision_assurance.jsonl` artifacts to the frozen snapshot, normalized cases, decisions, audit, diagnostics, comparisons, metrics, and run manifest. In both ordinary and descriptor-bound historical paths, every deterministic output is strict-parsed, structurally compared, and exact-digest frozen after write; raw engine decisions and audit are separately exact-frozen. The entire set is reverified before manifest construction, after construction, and after manifest write. The manifest is not self-hashed, and only successful harness return after those checks establishes completion. Generated replay outputs are local artifacts and should remain untracked. The harness refuses to overwrite a nonempty output directory.
...
- [`SOURCE_TO_DECISION_ASSURANCE.md`](SOURCE_TO_DECISION_ASSURANCE.md) defines the Phase 2.5 stage scope, deterministic numeric rule, artifact ordering, incomplete-run semantics, planned campaign, and nonclaims.
- [`RESEARCH_INFORMED_VALIDATION.md`](RESEARCH_INFORMED_VALIDATION.md) records the research-derived evaluation lessons applied to Phase 2.4 and Phase 2.5 and explicitly separates source facts from project recommendations.
