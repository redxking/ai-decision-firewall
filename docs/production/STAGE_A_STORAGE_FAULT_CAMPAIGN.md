...
| Filesystem/block device | System integration | A disposable privileged Linux lab switches an ext4 loopback device from `linear` to full-device `dm-error` at T1, post-effect observation, T2, audit closure, and T3. After restoring the mapping and running `e2fsck`, fresh Stage A construction preserves the exact receipt/effect boundary and returns conservative recovery or the durable T3 result. A separately selected `dm-flakey error_writes` mode is implemented and fails closed when the kernel target is unavailable; it is not yet observed evidence. | Execute `dm-flakey error_writes` on a capable kernel; then add `dm-flakey drop_writes`, torn writes, lost flushes, directory-entry loss, individual WAL/main-DB/audit faults, x86_64 repetition, and one intended CSI/filesystem. |
