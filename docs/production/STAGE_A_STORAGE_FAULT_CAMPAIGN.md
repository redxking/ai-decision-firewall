...
| Filesystem/block device | System integration | Disposable privileged Linux labs switch an ext4 loopback device from `linear` to either full-device `dm-error` or `dm-flakey error_writes` at T1, post-effect observation, T2, audit closure, and T3. After restoring the mapping and running `e2fsck`, fresh Stage A construction preserved the exact receipt/effect boundary and returned conservative recovery or the durable T3 result in all ten mode/boundary cases. The flakey result is exact-SHA GitHub-hosted development evidence; it is not independent or intended-environment proof. | Add `dm-flakey drop_writes`, torn writes, lost flushes, directory-entry loss, individual WAL/main-DB/audit faults, x86_64 repetition, and one intended CSI/filesystem. |
...
- `tests/container_stage_a_block_device_fault.py` runs only under the explicit
  ...
  network, and provides only loopback files on tmpfs to an explicitly selected
...
- `.github/workflows/stage-a-storage-lab.yml` is a manual-only, exact-SHA,
  read-token ephemeral Linux carrier for the flakey run. It validates
  the ordinary candidate first, requires the kernel target, never publishes an
  image, and retains bounded logs. Run 32423745805 passed all five flakey
  boundaries against candidate `662cb668f193667af37eddddf2040e666d188d76`;
  committing the workflow alone was not treated as evidence.
