...
The source-to-decision predecessor design-freeze implementation passed the 193-test review-local suite, and CI succeeded for exact Commit `08ce203c` on 2026-08-15. That supports CE-1 calculation-consistency wording for the exact commit only. The complete Phase 2.5 package was subsequently published on `main` at exact Commit `854b15c56397a81de6326b719d3d7d1dc847608f`; its exact-commit CI and Dependency Graph checks passed. That package adds bounded controls, documentation, visuals, and packaging outside the predecessor commit; its Phase 2.5 technical suite passed 222/222, the separate public-site module passed 9/9, and the then-current repository aggregate passed 231/231. The site module is outside this ADR and Phase 2.5 evidence. The package includes a generated-and-verified integrity manifest and inspected final-source status renders. The tracked data, campaign-bound model, and baseline outputs remain at committed bytes. Alpha.6 is not tagged and has no release or evidence package. `P2-CE-005` remains CE-0 `NOT_EVALUATED`.
