...
class RunResult:
    ...
    ever_compromised_agents: int
    ever_compromise_fraction: float
    peak_compromised_agents: int
    peak_compromise_fraction: float
...
def build_graph(n: int, topology: str, rng: random.Random) -> Dict[int, Set[int]]:
    ...
    if topology=="ring":
        for i in range(n): g[i].add((i+1)%n); g[i].add((i-1)%n)
    elif topology=="star":
...
def run_once(cfg: Config) -> RunResult:
    ...
    rng=random.Random(cfg.seed); graph=build_graph(cfg.n_agents,cfg.topology,rng); weights=privilege_weights(cfg,rng)
    ...
    ever=set(compromised); peak=len(compromised); mr=ua=alerts=useful=sq=0; exposed=0.0
    for t in range(cfg.steps):
        new=set()
        if cfg.topology=="dense" and compromised:
            p_eff=1.0-(1.0-cfg.propagation_probability)**len(compromised)
            for j in range(cfg.n_agents):
                if j not in compromised and rng.random()<p_eff: new.add(j)
        else:
        ...
        if new:
            ...
            last=t; compromised|=new; ever|=new; peak=max(peak,len(compromised))
        for i in range(cfg.n_agents):
            useful+=1; malicious=i in compromised and rng.random()<cfg.malicious_request_probability
            if malicious:
                mr+=1
                if rng.random()<cfg.policy_false_allow_rate: ua+=1; exposed+=weights[i]
                else: alerts+=1; sq+=1
            else: sq+=1
    ...
    return RunResult(cfg.seed,cfg.n_agents,cfg.topology,cfg.auth_model,len(compromised),len(compromised)/cfg.n_agents,len(ever),len(ever)/cfg.n_agents,peak,peak/cfg.n_agents,mr,ua,ail,blast,latency,alerts,useful,sq,ratio)
